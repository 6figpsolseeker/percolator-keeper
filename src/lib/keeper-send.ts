...
export async function keeperSend(
  ...
  try {
    ...
    try {
      ...
      release("success");
      // M12: after the tx confirms and the reservation is released, fetch the
      // actual on-chain cost and reconcile the budget. estimatedCost was what
      // the canSpend gate checked, but the realized cost (meta.fee) differs
      // because the actual priority fee depends on CU consumed × microLamports,
      // and the keeper may have under- or over-estimated the CU. Fire-and-forget
      // so the send return is not blocked. Errors are caught inside so a flaky
      // getTransaction never propagates here.
